...
- **FR-002**: 系统必须提供一个 `LoginModal` 组件，在需要鉴权时弹出，内容与独立登录页面相同（用户名、密码、登录按钮）
- **FR-003**: 系统必须在侧边栏导航菜单末尾显示 `UserStatus` 组件（用户状态模块）
- **FR-004**: `UserStatus` 组件在未登录时必须显示"未登录"文字和"登录"按钮，点击后弹出 `LoginModal`
- **FR-005**: `UserStatus` 组件在已登录时必须显示用户头像（默认占位符或已上传头像）、用户名和"登出"按钮
- **FR-006**: 系统必须在以下情况自动弹出 `LoginModal`：1) 用户点击需要鉴权的操作按钮，2) API返回401错误，3) 用户点击侧边栏"登录"按钮
...
- **FR-011**: 系统必须在 `LoginModal` 登录成功后自动重试之前失败的操作（如重新发起API请求或打开功能Modal）
- **FR-012**: 系统必须在token无效或过期时弹出 `LoginModal`（优先）或跳转到登录页面（备选）
...
- **FR-020**: 系统必须在HTTP拦截器中统一处理401响应，弹出 `LoginModal`

#### 路由保护需求

- **FR-021**: 系统必须定义哪些路由需要认证（如 `/accounts`, `/tasks`, `/notifications`, `/settings`）
- **FR-022**: 系统必须在路由守卫（Route Guard）中检查token有效性
- **FR-023**: 未登录用户访问受保护路由时，必须弹出 `LoginModal`（优先）或重定向到 `/login?redirect=<original-path>`（备选）
...
- **用户头像（User Avatar）**: 用户上传的头像图片或默认占位符，显示在 `UserStatus` 组件中
- **重定向路径（Redirect Path）**: 记录用户原本想访问的路径，独立登录页面登录成功后自动跳转
- **Pending Action**: 触发 `LoginModal` 时记录的待执行操作（如API请求或打开功能Modal），登录成功后自动重试
...
- **SC-004**: Token过期后，用户在下次操作时能在2秒内看到 `LoginModal`，并提示需要重新登录
- **SC-005**: 登录成功后，后续所有API请求都能正确携带Authorization header，无需手动处理
- **SC-006**: 退出登录后，所有受保护页面都无法访问，必须重新登录才能使用
- **SC-007**: 未登录用户直接访问受保护页面时，能在1秒内看到 `LoginModal` 或被重定向到登录页面
- **SC-008**: `LoginModal` 登录成功后，之前触发的操作能在2秒内自动重试并完成（如打开账号绑定Modal）
- **SC-009**: `UserStatus` 组件在登录状态变化时能在0.5秒内更新显示（登录后显示头像，退出后显示"未登录"）
- **SC-010**: 用户点击 `UserStatus` 组件的"登录"按钮后，`LoginModal` 能在0.3秒内弹出
...
- 登录页面采用居中卡片布局，简洁美观
- `LoginModal` 采用居中弹出设计，背景半透明遮罩，点击遮罩或关闭按钮可关闭
- `UserStatus` 组件位于侧边栏导航菜单末尾，作为最后一个菜单项
