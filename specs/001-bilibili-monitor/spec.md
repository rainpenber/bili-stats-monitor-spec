...
管理员可以添加要监控的“视频”或“博主”，系统会自动为这些任务设置定时数据抓取。
...
普通用户可以查看所有已监控的视频和博主的数据，但无法修改任务或系统配置。
...
4. **Given** 普通用户点击视频卡片, **When** 进入视频详情页, **Then** 显示两个折线图：播放量+在线观看人数、互动数据（弹幕、评论、投币、点赞）
5. **Given** 普通用户点击博主卡片, **When** 进入博主详情页, **Then** 显示粉丝数变化折线图，图表默认缩放到近3个月，纵轴范围为可见数据±500
...
2. **Given** 任务编辑表单已打开, **When** 修改定时策略为"固定频率"并设置为“数字+单位”的新间隔, **Then** 系统保存新配置
...
用户可以按博主、对象类型（视频/博主）或自定义标签对监控任务进行分类展示，支持多标签筛选。
...
1. **Given** 用户已登录且有多个监控任务, **When** 进入任务列表页面, **Then** 显示分类筛选面板（按博主、对象类型、标签）
2. **Given** 分类筛选面板已显示, **When** 选择特定博主, **Then** 任务列表只显示该博主的任务
3. **Given** 分类筛选面板已显示, **When** 选择对象类型为"视频", **Then** 任务列表只显示视频监控任务
...
用户可以通过关键字搜索监控任务，支持按视频BV号、博主UID、标题、博主名等搜索。
...
管理员可以为每个博主分别设置粉丝突增/突降的告警阈值，支持“绝对值”和“百分比”两种方式；默认关闭，可选择填写启用。
...
2. **Given** 粉丝告警设置已打开, **When** 设置粉丝增加告警阈值为绝对值或百分比并启用推送, **Then** 系统保存配置
3. **Given** 告警阈值已设置, **When** 粉丝数在默认24小时窗口内变化超过阈值, **Then** 系统触发告警并发送通知
...
- **FR-009**: 系统MUST支持两种监控任务类型：视频、博主
- **FR-010**: 系统MUST为每个监控任务关联一个B站账号（用于鉴权，未关联时仅获取公开数据）
- **FR-011**: 系统MUST支持两种定时策略：
  - 固定频率：采用“数字+单位”的组合填写方式（数字输入框+单位下拉：分钟/小时/天），最小间隔10分钟；最大值由系统配置项决定（默认1天）
  - 智能频率：以视频发布时间为起点，分为三个自然时间段：
    - 段A：[发布时刻, +5天) → 每10分钟
    - 段B：[+5天, +14天) → 每2小时
    - 段C：[+14天, 截止] → 每4小时
- **FR-012**: 系统MUST为每个监控任务设置3个月的默认截止时间，超过截止时间后自动停止监控
- **FR-013**: 系统MUST允许管理员修改监控任务的定时策略和截止时间
- **FR-014**: 系统MUST允许管理员删除监控任务（历史数据保留）
- **FR-015**: 系统MUST支持任务状态管理：监控中、已停止、已完成（达到截止）、失败（超过最大重试）
- **FR-016**: 系统MUST允许管理员手动启动或停止任务
- **FR-017**: 系统MUST使用队列机制处理数据抓取，确保同一时间仅有一个抓取在执行（全局单并发），避免并发请求冲突
...
- ~~**FR-021**: 系统MUST收集动态数据：评论数、转发数、点赞数（本期MVP不包含动态监控）~~
...
- ~~**FR-027**: 系统MUST在动态卡片显示：发布时间、评论数、转发数、点赞数（本期MVP不包含动态监控）~~
- **FR-028**: 系统MUST提供视频详情页，显示两个折线图：
  - 播放量和实时观看人数的变化趋势（从视频发布至今的全部数据）
  - 互动数据（弹幕、评论、投币、点赞）的变化趋势
- **FR-029**: 系统MUST提供博主详情页，显示粉丝数变化折线图，默认缩放到近3个月，纵轴范围为可见数据±500
- ~~**FR-030**: 系统MUST提供动态详情页，显示互动数据的变化趋势（本期MVP不包含动态监控）~~

#### 分类与搜索

- **FR-032**: 系统MUST支持按博主分类展示监控任务
- **FR-033**: 系统MUST支持按对象类型（视频/博主）分类展示监控任务
- **FR-034**: 系统MUST支持自定义标签，允许用户为任务添加多个标签
- **FR-035**: 系统MUST支持按标签筛选任务，支持多标签组合筛选
- **FR-036**: 系统MUST支持按关键字搜索，支持范围：视频BV号、博主UID、视频标题、博主名
...
- **FR-043**: 粉丝告警阈值MUST支持“绝对值”和“百分比”两种模式，默认关闭，可选择填写启用；默认对比窗口为24小时

#### 日志记录

- **FR-044**: 系统MUST记录三个等级的日志：DEBUG、INFO、WARNING
- **FR-045**: DEBUG级别MUST记录数据抓取过程的关键步骤（敏感信息需脱敏）
- **FR-046**: INFO级别MUST记录数据抓取、任务启停、通知发送等操作的详细时间和内容
- **FR-047**: WARNING级别MUST记录异常事件：数据抓取失败、推送失败、身份过期等
- **FR-048**: 系统MUST按日期保存日志为独立的日志文件
- **FR-049**: 系统MUST永久保留所有日志，并允许用户查看和下载日志文件

#### UI/UX

- **FR-050**: 系统MUST使用简约风格，参考 shadcn/ui 的视觉规范
- **FR-051**: 系统MUST采用左侧导航栏+主内容区的布局
- **FR-052**: 系统MUST在导航栏提供以下页面链接：仪表板、视频、博主、日志、设置、通知
- **FR-053**: 系统MUST使用卡片列表展示监控任务，支持分页
- **FR-054**: 在任务表单中，固定频率输入MUST采用“数字输入框+单位下拉（分钟/小时/天）”的交互方式

### Key Entities

- **User**: 系统用户，包含用户名、密码、角色（管理员/普通用户）
- **BiliAccount**: B站账号，包含UID、Cookie（SESSDATA）、绑定时间、状态（有效/过期）
- **MonitoringTask**: 监控任务，包含任务ID、任务类型（视频/博主）、目标ID（BV号/UID等）、关联账号、定时策略、截止时间、状态、创建时间、标签列表
- **VideoData**: 视频数据，包含视频ID、采集时间、播放量、实时观看人数、弹幕数、评论数、投币数、点赞数、播放完成率（可选）、平均观看时长（可选）
- **AuthorData**: 博主数据，包含博主UID、采集时间、粉丝数
- ~~**DynamicData**: 动态数据（本期MVP不包含动态监控）~~
...
- **SC-003**: 系统能够稳定处理≥100个活跃监控任务（队列无明显堆积，失败率可控）
- **SC-004**: 任务队列处理效率：单次抓取平均处理时间不超过30秒
- **SC-005**: 系统能够永久保存一年数据且常用查询在5秒内返回（以当前规模估算：<30万条/年）
- **SC-006**: 仪表板加载时间不超过3秒
...
1. **数据量估算（按MVP范围）**：
   ...
   - 计算：前5天 = 5 × 24 × 6 = 720条；5~14天 = 9 × 12 = 108条；14~365天 = 351 × 6 = 2106条
   - 单个视频年数据量 ≈ 2934条；10个视频 ≈ 29340条
   - 博主粉丝数：每天1条，365条/年；5个博主 = 1825条
   - **总计：约31165条记录/年**（规模可通过SQLite等轻量数据库支撑；实际随任务数量变化）

2. **Cookie安全存储**：假设采用安全的加密存储和访问控制

3. **B站接口可用性**：假设B站公开/非公开接口在合理范围内稳定可用

4. **系统时区**：所有时间展示采用北京时间（UTC+8）

5. **队列与并发**：采用全局单并发抓取，避免并发导致的风控和限流

---

## Notes

- 本规范关注功能需求与业务规则，不涉及具体的技术实现细节
- 智能频率的边界采用自然时间段：[发布, +5天)、[+5天, +14天)、[+14天, 截止]
- 固定频率采用“数字+单位”的输入方式，最小10分钟，最大值由系统配置项控制（默认1天）
- 粉丝告警阈值默认关闭，支持绝对值/百分比两种；默认对比窗口24小时
- 动态监控不在本期MVP范围内，相关需求已移除
