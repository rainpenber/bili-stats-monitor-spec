...
2. **Given** 任务类型为“视频”并输入BV号或“博主”并输入UID, **When** 提交, **Then** 系统创建任务（前端不强制校验格式）
3. **Given** 任务已创建, **When** 返回卡片列表, **Then** 新卡片显示视频封面（16:9）/博主头像（1:1）及核心数字（视频：播放量“xx万”；博主：粉丝“xx万”）
4. **Given** 在卡片列表, **When** 点击任一卡片, **Then** 页面下方“详情区”展开并显示图表；再次点击其它卡片可快速切换详情区内容
5. **Given** 卡片右下角菜单按钮, **When** 点击, **Then** 弹出编辑任务Modal（卡片不直接显示“删除”按钮）
...
- **FR-014**: 系统MUST为每个监控任务可选关联一个B站账号（用于鉴权；未关联则仅获取公开数据）
...
- **FR-038**: 粉丝告警阈值MUST支持“绝对值/百分比”两种模式，默认关闭，默认对比窗口24小时

#### 日志记录

- **FR-039**: 系统MUST记录三个等级的日志：DEBUG、INFO、WARNING
- **FR-040**: DEBUG需记录抓取关键步骤（敏感信息脱敏）
- **FR-041**: INFO需记录抓取、任务启停、批量操作、通知发送等事件
- **FR-042**: WARNING需记录抓取失败、通知失败、鉴权过期等异常
- **FR-043**: 日志MUST按日期滚动保存并允许下载
